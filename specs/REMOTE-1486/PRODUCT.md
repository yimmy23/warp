...
## Goals
- A user in a local Oz agent conversation can hand off to a cloud agent without leaving their flow — clicking the "Hand off to cloud" chip (or running `/oz-cloud-handoff`) opens a split cloud-mode pane next to the local agent, where the user types a follow-up prompt and submits.
...
## Non-goals
- Third-party harnesses (Claude Code, Gemini, etc.). A parallel `/claude-cloud-handoff` is a follow-up that reuses most of the plumbing.
...
## Behavior
### Entry points
1. A "Hand off to cloud" chip is added to the agent input footer's right slot whenever `FeatureFlag::OzHandoff && FeatureFlag::LocalToCloudHandoff` are both enabled, in agent-view panes only, and not for session viewers (handoff is host-initiated). The chip uses the existing `bundled/svg/upload-cloud-01.svg` icon (cloud-with-upward-arrow); design may swap this for a bespoke icon as a follow-up.
2. A slash command `/oz-cloud-handoff [optional prompt]` is registered under the same flag gates plus the existing `AGENT_VIEW | ACTIVE_CONVERSATION | AI_ENABLED` availability rules. The name is Oz-specific so a future `/claude-cloud-handoff` can ship alongside.
...
11. When the user submits the prompt, the client (off the main thread):
    ...
    4. Calls `POST /agent/runs` (`SpawnAgentRequest`) with `fork_from_conversation_id` + `handoff_prep_token` set. The server forks the source conversation, creates the new task, and moves the prep blobs into the task's snapshot prefix.
